...
🦀 **TabbyMew** 是一个轻量、高效、Agent 友好的代理应用，主要面向
macOS 和 Windows。它保持核心运行时足够小，提供稳定的 CLI/TUI 控制面，
把状态写入可预测的位置，并使用严格的原生 JSON 配置格式。

TabbyMew 关注这些特性：

- ⚡ **轻量运行时**：一个 Rust 核心服务负责监听、路由、TUN、系统代理、
  订阅、日志和清理。
- 🧭 **高效控制**：人类用户可以直接进入 TUI，Agent 和自动化流程可以使用
  稳定的 `--json` CLI 输出。
- 🤖 **Agent 友好**：命令入口明确，错误码稳定，诊断结果可机器读取，并提供
  可直接执行的 `next_actions`。
- 🖥️ **跨平台桌面目标**：macOS 和 Windows 是主要支持平台；Linux 主要用于
  开发和验证。
...
| 类型 | TCP | UDP | 说明 |
| --- | --- | --- | --- |
| SOCKS5 | ✅ | ✅ | TCP connect 和 UDP associate |
| HTTP CONNECT | ✅ | ❌ | 适合浏览器和系统代理 |
| Hybrid | ✅ | SOCKS | 一个本地端口同时支持 SOCKS5 和 HTTP |
| TUN | ✅ | ✅ | 设备级路由，需要管理员/root 权限 |
...
```yaml
human_protocol_matrix: README.zh-CN.md#protocol-support
